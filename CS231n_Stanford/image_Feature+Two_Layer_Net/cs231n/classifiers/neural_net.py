...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    for _ in range(relu_1_activation.shape[0]):
      WX.append(list(range(W2.shape[1])))

    for j in range(W2.shape[1]):
      for i in range(relu_1_activation.shape[0]):
        sum = 0
        for k in range(relu_1_activation.shape[1]):
          sum += relu_1_activation[i][k] * W2[k][j]
        WX[i][j] = sum
    WX = np.array(WX)
    ...
    for row in WX:
      ...
      for i in range(WX.shape[1]):
        new_row.append(row[i] + b2[i])
      WX2.append(new_row)

    fc2_activation = np.array(WX2)
    ...
    for idx, row in enumerate(shift_scores):
      sum = 0
      for i in range(shift_scores.shape[1]):
        sum += math.exp(row[i])
      los.append(math.log(sum) - correct_class_scores[idx])
    ...
    for _ in range(relu_1_activation.T.shape[0]):
      dW2.append(list(range(dSoft.shape[1])))

    for j in range(dSoft.shape[1]):
      for i in range(relu_1_activation.T.shape[0]):
        sum = 0
        for k in range(relu_1_activation.T.shape[1]):
          sum += relu_1_activation.T[i][k] * dSoft[k][j]
        dW2[i][j] = sum
    dW2 = np.array(dW2)
    ...
    for col in range(db2.shape[1]):
      sum7 = 0
      for i in db2[:, col]:
        sum7 += i
      bb.append(sum7)

    grads['b2'] = np.array(bb).T

    # dx2 = np.dot(dSoft, W2.T)
    # 문제 3-3: 위 구문(line: 149)을 numpy lib를 사용하지 않고 numpy lib를 사용한 결과와 동일하게 동작하도록 작성
    dx2 = []

    for _ in range(dSoft.shape[0]):
      dx2.append(list(range(W2.T.shape[1])))

    for j in range(W2.T.shape[1]):
      for i in range(dSoft.shape[0]):
        sum = 0
        for k in range(dSoft.shape[1]):
          sum += dSoft[i][k] * W2.T[k][j]
        dx2[i][j] = sum
    dx2 = np.array(dx2)
    ...
    for _ in range(X.T.shape[0]):
      dW1.append(list(range(dRelu1.shape[1])))

    for j in range(dRelu1.shape[1]):
      for i in range(X.T.shape[0]):
        sum = 0
        for k in range(X.T.shape[1]):
          sum += X.T[i][k] * dRelu1[k][j]
        dW1[i][j] = sum
    dW1 = np.array(dW1)
    ...
    for col in range(db1.shape[1]):
      sum8 = 0
      for i in db1[:, col]:
        sum8 += i
      b.append(sum8)

    grads['b1'] = np.array(b).T
  ...
  def predict(self, X):
    ...
    pred = []
    for row in range(self.loss(X).shape[0]):
      max = float("-inf")
      for idx, value in enumerate(self.loss(X)[row, :]):
        if value > max:
          max = value
          max_idx = idx
      pred.append(max_idx)

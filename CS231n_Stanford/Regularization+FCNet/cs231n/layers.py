...
def affine_forward(x, w, b):
    ...
    if total % new_row == 0:
        for i in range(new_row):
            reshape.append(xf[(i * int(total / new_row)): (i + 1) * int(total / new_row)])

    reshaped_input = np.array(reshape)

    rw = []
    for _ in range(reshaped_input.shape[0]):
        rw.append([0 for _ in range(w.shape[1])])

    for j in range(w.shape[1]):
        for i in range(reshaped_input.shape[0]):
            sum = 0
            for k in range(reshaped_input.shape[1]):
                sum += reshaped_input[i][k] * w[k][j]
            rw[i][j] = sum
...
def affine_backward(dout, cache):
    ...
    if total % new_row == 0:
        for i in range(new_row):
            reshape.append(x_ff[(i * int(total / new_row)): (i + 1) * int(total / new_row)])

    reshaped_x = np.array(reshape)


    dow = []
    for _ in range(dout.shape[0]):
        dow.append(list(range(w.T.shape[1])))

    for j in range(w.T.shape[1]):
        for i in range(dout.shape[0]):
            sum = 0
            for k in range(dout.shape[1]):
                sum += dout[i][k] * w.T[k][j]
            dow[i][j] = sum
    ...
    dx = np.reshape(dx, x.shape)
    rdo = []
    for _ in range(reshaped_x.T.shape[0]):
        rdo.append(list(range(dout.shape[1])))

    for j in range(dout.shape[1]):
        for i in range(reshaped_x.T.shape[0]):
            sum = 0
            for k in range(reshaped_x.T.shape[1]):
                sum += reshaped_x.T[i][k] * dout[k][j]
            rdo[i][j] = sum
    ...
    for col in range(dout.shape[1]):
        sum = 0
        for item in dout[:, col]:
            sum += item
        fs.append(sum)
    db = np.array(fs)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        for col in range(x.shape[1]):
            ...
            for item in x[:, col]:
                sum += item
            mx.append(sum / x.shape[0])
        sample_mean = np.array(mx)
        ...
        for col in range(x.shape[1]):
            sum = 0
            for item in x[:, col]:
                sum += (item - sample_mean[col]) ** 2
            vx.append(sum / x.shape[0])
        sample_var = np.array(vx)


        varx_sqrt = []
        for i in sample_var:
            varx_sqrt.append(math.sqrt(i + eps))

        normalized_data = (x - sample_mean) / np.array(varx_sqrt)
        out = gamma * normalized_data + beta
        ...
        cache = {
            ...
            'normalized_data': normalized_data,
            'gamma': gamma,
            'ivar': 1. / np.array(varx_sqrt),
            'sqrtvar': np.array(varx_sqrt),
        }
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        ...
        rvar_sqrt = []
        for i in running_var:
            rvar_sqrt.append(math.sqrt(i + eps))

        out = (gamma / np.array(rvar_sqrt) * x) + (beta - (gamma * running_mean) / np.array(rvar_sqrt))
...
def batchnorm_backward(dout, cache):
    ...
    for col in range( (dout * normalized_data).shape[1] ):
        sum = 0
        for item in (dout * normalized_data)[:, col]:
            sum += item
        fns.append(sum)
    dgamma = np.array(fns)
    ...
    for col in range((dxhat * x_minus_mean).shape[1]):
        sum = 0
        for item in (dxhat * x_minus_mean)[:, col]:
            sum += item
        fhs.append(sum)
    divar = np.array(fhs)
    ...
    dsq = (1 / N) * dvar * np.array([ [1. for _ in range(dout.shape[1])] for _ in range(dout.shape[0]) ])
    ...
    for col in range((dxmu1 + dxmu2).shape[1]):
        sum = 0
        for item in (dxmu1 + dxmu2)[:, col]:
            sum += item
        fxs.append(sum)
    dmu = -1 * np.array(fxs)


    dx2 = (1 / N) * dmu * np.array([ [1. for _ in range(dout.shape[1])] for _ in range(dout.shape[0]) ])
...
def batchnorm_backward_alt(dout, cache):
    ...
    for col in range((dout * (x_minus_mean)).shape[1]):
        sum = 0
        for item in (dout * (x_minus_mean))[:, col]:
            sum += item
        fxms.append(sum)
    fxms = np.array(fxms)

    dx = (1 / N) * gamma * 1 / sqrtvar * ((N * dout) - dbeta - (x_minus_mean) * np.square(ivar) * fxms)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = ( np.array([[random.random() for _ in range(x.shape[1])] for _ in range(x.shape[0])] ) >= p)

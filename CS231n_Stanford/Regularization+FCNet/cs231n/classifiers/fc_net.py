...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ## 문제 1. end block ###############################################################################
        self.params['b1'] = np.array([0 for _ in range(hidden_dim)])
        self.params['b2'] = np.array([0 for _ in range(num_classes)])

        self.params['W1'] = np.array([[random.gauss(0, weight_scale) for _ in range(hidden_dim)] for _ in range(input_dim)])
        self.params['W2'] = np.array([[random.gauss(0, weight_scale) for _ in range(num_classes)] for _ in range(hidden_dim)])
    ...
    def loss(self, X, y=None):
        ...
        ss_w1 = 0
        for row in self.params['W1']:
            for item in row:
                ss_w1 += item ** 2

        ss_w2 = 0
        for row in self.params['W2']:
            for item in row:
                ss_w2 += item ** 2

        loss += 0.5*self.reg*(ss_w1 + ss_w2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ## 문제 3. start block #############################################################################
        for i in range(self.num_layers - 1):
            self.params['W' + str(i+1)] = np.array([ [random.gauss(0, weight_scale) for _ in range(hidden_dims[i])] for _ in range(input_dim) ])
            self.params['b' + str(i+1)] = np.array([ 0 for _ in range(hidden_dims[i])])

            if self.use_batchnorm:
                self.params['beta' + str(i+1)] = np.array([ 0 for _ in range(hidden_dims[i])])
                self.params['gamma' + str(i+1)] = np.array([ 1 for _ in range(hidden_dims[i])])
    ...
    def loss(self, X, y=None):
        ...
        ## 문제 4-1. end block #############################################################################
        ss_last = 0
        for row in self.params['W'+str(self.num_layers)]:
            for item in row:
                ss_last += item ** 2
        loss += 0.5 * self.reg * ss_last
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            ## 문제 4-1. end block #############################################################################
            ss_back = 0
            for row in self.params['W' + str(i)]:
                for item in row:
                    ss_back += item ** 2
            loss += 0.5 * self.reg * ss_back
